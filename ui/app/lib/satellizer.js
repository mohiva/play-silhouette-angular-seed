    ...
    .factory('satellizer.storage', ['satellizer.config', function(config) {
      var storage = function(type) {
        var stub = {
          ...
          removeItem: function(key) { return undefined; }
        };
        ...
        switch (type) {
          ...
          case 'sessionStorage':
            if (type in window && window[type] !== null) {
              ...
              console.warn('Warning: Storage type `' + type + '` is disabled or unavailable. Satellizer will not work correctly.');
              return stub;
            }

          default:
            console.error('Error: Unexpected storage type `' + type + '`');
            return stub;
        }
      };

      return {
        get: function(key) { return storage(config.storage).getItem(key); },
        set: function(key, value) { return storage(config.storage).setItem(key, value); },
        remove: function(key) { return storage(config.storage).removeItem(key); }
      };
    }])
